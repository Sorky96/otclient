...
public:
	...
	static uint64_t getMaxMeleeDamage(uint64_t attackSkill, uint64_t attackValue);
	static uint64_t getMaxWeaponDamage(uint32_t level, uint64_t attackSkill, uint64_t attackValue, float attackFactor);
...
public:
	...
	virtual uint64_t getWeaponDamage(const Player* player, const Creature* target, const Item* item,
	                                bool maxDamage = false) const = 0;
	virtual uint64_t getElementDamage(const Player* player, const Creature* target, const Item* item) const = 0;
	...
	void setHealth(uint64_t h) { health = h; }
...
protected:
	void internalUseWeapon(Player* player, Item* item, Creature* target, uint64_t damageModifier) const;
...
private:
	...
	uint64_t getHealthCost(const Player* player) const;
...
public:
	...
	uint64_t getWeaponDamage(const Player* player, const Creature* target, const Item* item,
	                        bool maxDamage = false) const override;
	uint64_t getElementDamage(const Player* player, const Creature* target, const Item* item) const override;
...
public:
	...
	uint64_t getWeaponDamage(const Player* player, const Creature* target, const Item* item,
	                        bool maxDamage = false) const override;
	uint64_t getElementDamage(const Player*, const Creature*, const Item*) const override { return 0; }
