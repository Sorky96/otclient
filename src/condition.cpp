...
bool Condition::setParam(ConditionParam_t param, uint64_t value)
{
	switch (param) {
...
uint64_t Condition::getParam(ConditionParam_t param)
{
	switch (param) {
		case CONDITION_PARAM_TICKS:
			return ticks;
...
void Condition::setTicks(uint64_t newTicks)
{
	ticks = newTicks;
	endTime = ticks + OTSYS_TIME();
}

bool Condition::executeCondition(Creature*, uint64_t interval)
{
	if (ticks == -1) {
		return true;
	}
...
Condition* Condition::createCondition(ConditionId_t id, ConditionType_t type, uint64_t ticks, uint64_t param /* = 0*/,
...
bool ConditionGeneric::executeCondition(Creature* creature, uint64_t interval)
{
	...
	for (int32_t i = STAT_FIRST; i <= STAT_LAST; ++i) {
		...
		switch (i) {
			case STAT_MAXHITPOINTS:
				stats[i] = static_cast<uint64_t>(player->getMaxHealth() * ((statsPercent[i] - 100) / 100.f));
				break;

			case STAT_MAXMANAPOINTS:
				stats[i] = static_cast<uint64_t>(player->getMaxMana() * ((statsPercent[i] - 100) / 100.f));
				break;
...
bool ConditionAttributes::executeCondition(Creature* creature, uint64_t interval)
{
...
bool ConditionAttributes::setParam(ConditionParam_t param, uint64_t value)
{
	...
	switch (param) {
		...
		case CONDITION_PARAM_STAT_MAXHITPOINTSPERCENT: {
			statsPercent[STAT_MAXHITPOINTS] = std::max<uint64_t>(0, value);
			return true;
		}

		case CONDITION_PARAM_STAT_MAXMANAPOINTSPERCENT: {
			statsPercent[STAT_MAXMANAPOINTS] = std::max<uint64_t>(0, value);
			return true;
		}
...
uint64_t ConditionAttributes::getParam(ConditionParam_t param)
{
	switch (param) {
		case CONDITION_PARAM_SKILL_FIST:
...
bool ConditionRegeneration::executeCondition(Creature* creature, uint64_t interval)
{
	...
	if (internalHealthTicks >= healthTicks) {
		...
		uint64_t realHealthGain = creature->getHealth();
	...
	if (internalManaTicks >= manaTicks) {
		...
		if (Player* player = creature->getPlayer()) {
			uint64_t realManaGain = player->getMana();
...
bool ConditionRegeneration::setParam(ConditionParam_t param, uint64_t value)
{
	bool ret = ConditionGeneric::setParam(param, value);
...
uint64_t ConditionRegeneration::getParam(ConditionParam_t param)
{
	switch (param) {
		case CONDITION_PARAM_HEALTHGAIN:
			return healthGain;
...
bool ConditionSoul::executeCondition(Creature* creature, uint64_t interval)
{
...
bool ConditionSoul::setParam(ConditionParam_t param, uint64_t value)
{
	bool ret = ConditionGeneric::setParam(param, value);
	switch (param) {
...
uint64_t ConditionSoul::getParam(ConditionParam_t param)
{
	switch (param) {
		case CONDITION_PARAM_SOULGAIN:
			return soulGain;
...
bool ConditionDamage::setParam(ConditionParam_t param, uint64_t value)
{
...
uint64_t ConditionDamage::getParam(ConditionParam_t param)
{
	switch (param) {
		case CONDITION_PARAM_OWNER:
			return owner;
...
bool ConditionDamage::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	if (attr == CONDITIONATTR_DELAYED) {
		...
		return propStream.read<uint64_t>(periodDamage);
...
void ConditionDamage::serialize(PropWriteStream& propWriteStream)
{
	...
	propWriteStream.write<uint64_t>(periodDamage);
...
bool ConditionDamage::addDamage(uint64_t rounds, uint64_t time, uint64_t value)
{
...
bool ConditionDamage::init()
{
	...
	uint64_t amount = uniform_random(minDamage, maxDamage);
	if (amount != 0) {
		if (startDamage > maxDamage) {
			startDamage = maxDamage;
		} else if (startDamage == 0) {
			startDamage = std::max<uint64_t>(1, std::ceil(amount / 20.0));
		}
...
bool ConditionDamage::executeCondition(Creature* creature, uint64_t interval)
{
	if (periodDamage != 0) {
		...
		if (damageInfo.timeLeft <= 0) {
			uint64_t damage = damageInfo.value;
...
bool ConditionDamage::getNextDamage(uint64_t& damage)
{
	if (periodDamage != 0) {
		damage = periodDamage;
		return true;
...
bool ConditionDamage::doDamage(Creature* creature, uint64_t healthChange)
{
	...
	if (field && creature->getPlayer() && attacker && attacker->getPlayer()) {
		damage.primary.value = static_cast<uint64_t>(std::round(damage.primary.value / 2.));
	}
...
uint64_t ConditionDamage::getTotalDamage() const
{
	int32_t result;
	if (!damageList.empty()) {
		result = 0;
...
void ConditionDamage::generateDamageList(uint64_t amount, uint64_t start, std::list<uint64_t>& list)
{
...
bool ConditionSpeed::setParam(ConditionParam_t param, uint64_t value)
{
...
uint64_t ConditionSpeed::getParam(ConditionParam_t param)
{
	switch (param) {
...
bool ConditionSpeed::executeCondition(Creature* creature, uint64_t interval)
{
	return Condition::executeCondition(creature, interval);
}
...
bool ConditionOutfit::executeCondition(Creature* creature, uint64_t interval)
{
	return Condition::executeCondition(creature, interval);
}
...
bool ConditionLight::executeCondition(Creature* creature, uint64_t interval)
{
...
bool ConditionLight::setParam(ConditionParam_t param, uint64_t value)
{
	bool ret = Condition::setParam(param, value);
	if (ret) {
		return false;
...
uint64_t ConditionLight::getParam(ConditionParam_t param)
{
	switch (param) {
		case CONDITION_PARAM_LIGHT_LEVEL:
...
bool ConditionDrunk::setParam(ConditionParam_t param, uint64_t value)
{
	bool ret = Condition::setParam(param, value);
...
uint64_t ConditionManaShield::onDamageTaken(Player* player, uint64_t manaChange)
{
	if (!player) {
		return 0;
	}
...
bool ConditionManaShield::setParam(ConditionParam_t param, uint64_t value)
{
	bool ret = Condition::setParam(param, value);
