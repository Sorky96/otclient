...
struct spellBlock_t
{
	...
	uint32_t speed = 2000;
	uint32_t range = 0;
	uint64_t minCombatValue = 0;
	uint64_t maxCombatValue = 0;
...
class MonsterType
{
	struct MonsterInfo
	{
		...
		std::map<CombatType_t, uint64_t> elementMap;
...
public:
	...
	int32_t radius = 0;
	int32_t ring = 0;
	uint64_t conditionMinDamage = 0;
	uint64_t conditionMaxDamage = 0;
	uint64_t conditionStartDamage = 0;
...
private:
	ConditionDamage* getDamageCondition(ConditionType_t conditionType, uint64_t maxDamage, uint64_t minDamage,
