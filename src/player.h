...
static constexpr uint32_t MINIMUM_SKILL_LEVEL = 10;
...
public:
	...
	{
		return std::max<uint64_t>(0, specialMagicLevelSkill[combatTypeToIndex(type)]);
	}
	...
	uint64_t getMaxHealth() const override { return std::max<uint64_t>(1, healthMax + varStats[STAT_MAXHITPOINTS]); }
	uint32_t getMana() const { return mana; }
	uint32_t getMaxMana() const { return std::max<uint64_t>(0, manaMax + varStats[STAT_MAXMANAPOINTS]); }
	...
	void setSpecialMagicLevelSkill(CombatType_t type, uint32_t modifier)
	{
	...
	void changeHealth(uint64_t healthChange, bool sendHealthChange = true) override;
	void changeMana(uint64_t manaChange);
	void changeSoul(int32_t soulChange);

	bool isPzLocked() const { return pzLocked; }
	BlockType_t blockHit(Creature* attacker, CombatType_t combatType, uint64_t& damage, bool checkDefense = false,
	...
	void drainHealth(Creature* attacker, uint64_t damage) override;
	void drainMana(Creature* attacker, uint64_t manaLoss);
	...
	void onAttackedCreatureDrainHealth(Creature* target, uint64_t points) override;
	void onTargetCreatureGainHealth(Creature* target, uint64_t points) override;
...
private:
	...
	std::array<uint32_t, COMBAT_COUNT> specialMagicLevelSkill = {0};
	int32_t purchaseCallback = -1;
	int32_t saleCallback = -1;
	int32_t MessageBufferCount = 0;
	uint64_t bloodHitCount = 0;
	...
	static uint8_t getBasisPointLevel(uint64_t count, uint64_t nextLevelCount);
