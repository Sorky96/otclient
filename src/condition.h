...
public:
	Condition() = default;
	Condition(ConditionId_t id, ConditionType_t type, uint64_t ticks, bool buff = false, uint32_t subId = 0,
	...
	virtual bool executeCondition(Creature* creature, uint64_t interval);
	...
	static Condition* createCondition(ConditionId_t id, ConditionType_t type, uint64_t ticks, uint64_t param = 0,
	                                  bool buff = false, uint32_t subId = 0, bool aggressive = false);
	static Condition* createCondition(PropStream& propStream);

	virtual bool setParam(ConditionParam_t param, uint64_t value);
	virtual uint64_t getParam(ConditionParam_t param);
...
{
public:
	ConditionGeneric(ConditionId_t id, ConditionType_t type, uint64_t ticks, bool buff = false, uint32_t subId = 0,
	                 bool aggressive = false) :
	    Condition(id, type, ticks, buff, subId, aggressive)
	{}

	bool startCondition(Creature* creature) override;
	bool executeCondition(Creature* creature, uint64_t interval) override;
...
{
public:
	ConditionAttributes(ConditionId_t id, ConditionType_t type, uint64_t ticks, bool buff = false, uint32_t subId = 0,
	                    bool aggressive = false) :
	...
	bool setParam(ConditionParam_t param, uint64_t value) override;
	uint64_t getParam(ConditionParam_t param) override;
...
{
public:
	ConditionRegeneration(ConditionId_t id, ConditionType_t type, uint64_t ticks, bool buff = false, uint32_t subId = 0,
	                      bool aggressive = false) :
	    ConditionGeneric(id, type, ticks, buff, subId, aggressive)
	{}
...
{
public:
	ConditionSoul(ConditionId_t id, ConditionType_t type, uint64_t ticks, bool buff = false, uint32_t subId = 0,
	              bool aggressive = false) :
	    ConditionGeneric(id, type, ticks, buff, subId, aggressive)
	{}
...
{
public:
	ConditionInvisible(ConditionId_t id, ConditionType_t type, uint64_t ticks, bool buff = false, uint32_t subId = 0,
	                   bool aggressive = false) :
	    ConditionGeneric(id, type, ticks, buff, subId, aggressive)
	{}
...
public:
	...
	static void generateDamageList(uint64_t amount, uint64_t start, std::list<uint64_t>& list);
	...
	bool addDamage(uint64_t rounds, uint64_t time, uint64_t value);
	bool doForceUpdate() const { return forceUpdate; }
	uint64_t getTotalDamage() const;

	void setInitDamage(uint64_t initDamage) { this->initDamage = initDamage; }
...
private:
	...
	uint64_t minDamage = 0;
	uint64_t startDamage = 0;
	uint64_t periodDamage = 0;
	uint64_t periodDamageTick = 0;
	int32_t tickInterval = 2000;
	uint64_t initDamage = 0;
	...
	bool getNextDamage(uint64_t& damage);
	bool doDamage(Creature* creature, uint64_t healthChange);
...
{
public:
	ConditionSpeed(ConditionId_t id, ConditionType_t type, uint64_t ticks, bool buff, uint32_t subId,
...
{
public:
	ConditionOutfit(ConditionId_t id, ConditionType_t type, uint64_t ticks, bool buff = false, uint32_t subId = 0,
	                bool aggressive = false) :
	    Condition(id, type, ticks, buff, subId, aggressive)
	{}
...
{
public:
	ConditionLight(ConditionId_t id, ConditionType_t type, uint64_t ticks, bool buff, uint32_t subId, uint8_t lightlevel,
...
{
public:
	ConditionDrunk(ConditionId_t id, ConditionType_t type, uint64_t ticks, bool buff, uint32_t subId,
...
{
public:
	ConditionManaShield(ConditionId_t initId, ConditionType_t initType, uint64_t iniTicks, bool initBuff = false,
	...
	void serialize(PropWriteStream& propWriteStream) override;
	bool unserializeProp(ConditionAttr_t attr, PropStream& propStream) override;
	uint64_t onDamageTaken(Player* player, uint64_t manaChange);
