...
ConditionDamage* Monsters::getDamageCondition(ConditionType_t conditionType, uint64_t maxDamage, uint64_t minDamage,
                                              uint64_t startDamage, uint32_t tickInterval)
{
...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if ((attr = node.attribute("min"))) {
		sb.minCombatValue = pugi::cast<uint64_t>(attr.value());
	}

	if ((attr = node.attribute("max"))) {
		sb.maxCombatValue = pugi::cast<uint64_t>(attr.value());

		// normalize values
		if (std::abs(sb.minCombatValue) > std::abs(sb.maxCombatValue)) {
			uint64_t value = sb.maxCombatValue;
	...
	if (isScripted) {
		...
		if (tmpName == "melee") {
			...
			if ((attackAttribute = node.attribute("attack")) && (skillAttribute = node.attribute("skill"))) {
				sb.minCombatValue = 0;
				sb.maxCombatValue = -Weapons::getMaxMeleeDamage(pugi::cast<uint64_t>(skillAttribute.value()),
				                                                pugi::cast<uint64_t>(attackAttribute.value()));
			}
			...
			if ((attr = node.attribute("fire"))) {
				...
				minDamage = pugi::cast<uint64_t>(attr.value());
				maxDamage = minDamage;
				tickInterval = 9000;
			...
			uint64_t minDamage = std::abs(sb.minCombatValue);
			uint64_t maxDamage = std::abs(sb.maxCombatValue);
...
bool Monsters::deserializeSpell(MonsterSpell* spell, spellBlock_t& sb, const std::string& description)
{
	...
	if (spell->isScripted) {
		...
		if (spell->conditionType != CONDITION_NONE) {
			...
			uint64_t conMinDamage = std::abs(spell->conditionMinDamage);
			uint64_t conMaxDamage = std::abs(spell->conditionMaxDamage);
			uint64_t startDamage = std::abs(spell->conditionStartDamage);
	...
	if ((node = monsterNode.child("flags"))) {
		for (auto flagNode : node.children()) {
			...
			if (caseInsensitiveEqual(attrName, "summonable")) {
				...
				mType->info.runAwayHealth = pugi::cast<uint64_t>(attr.value());
	...
	if ((node = monsterNode.child("elements"))) {
		for (auto elementNode : node.children()) {
			if ((attr = elementNode.attribute("physicalPercent"))) {
				mType->info.elementMap[COMBAT_PHYSICALDAMAGE] = pugi::cast<uint64_t>(attr.value());
				...
				mType->info.elementMap[COMBAT_ICEDAMAGE] = pugi::cast<uint64_t>(attr.value());
				...
				mType->info.elementMap[COMBAT_EARTHDAMAGE] = pugi::cast<uint64_t>(attr.value());
				...
				mType->info.elementMap[COMBAT_FIREDAMAGE] = pugi::cast<uint64_t>(attr.value());
				...
				mType->info.elementMap[COMBAT_ENERGYDAMAGE] = pugi::cast<uint64_t>(attr.value());
				...
				mType->info.elementMap[COMBAT_HOLYDAMAGE] = pugi::cast<uint64_t>(attr.value());
				...
				mType->info.elementMap[COMBAT_DEATHDAMAGE] = pugi::cast<uint64_t>(attr.value());
				...
				mType->info.elementMap[COMBAT_DROWNDAMAGE] = pugi::cast<uint64_t>(attr.value());
				...
				mType->info.elementMap[COMBAT_LIFEDRAIN] = pugi::cast<uint64_t>(attr.value());
				...
				mType->info.elementMap[COMBAT_MANADRAIN] = pugi::cast<uint64_t>(attr.value());
