...
void tfs::lua::pushPosition(lua_State* L, const Position& position, uint64_t stackpos /* = 0*/)
{
	lua_createtable(L, 0, 4);
...
int LuaScriptInterface::luaDoAreaCombat(lua_State* L)
{
	...
	if (area || areaId == 0) {
		...
		damage.primary.value = normal_random(tfs::lua::getNumber<uint64_t>(L, 5), tfs::lua::getNumber<uint64_t>(L, 6));
...
int LuaScriptInterface::luaCreatureSetHealth(lua_State* L)
{
	...
	creature->health = std::min<uint64_t>(tfs::lua::getNumber<uint32_t>(L, 2), creature->healthMax);
...
int LuaScriptInterface::luaCreatureAddHealth(lua_State* L)
{
	...
	CombatDamage damage;
	damage.primary.value = tfs::lua::getNumber<uint64_t>(L, 2);
...
int LuaScriptInterface::luaCreatureSetMaxHealth(lua_State* L)
{
	...
	creature->health = std::min<uint64_t>(creature->health, creature->healthMax);
	g_game.addCreatureHealth(creature);
...
int LuaScriptInterface::luaPlayerAddMana(lua_State* L)
{
	...
	uint64_t manaChange = tfs::lua::getNumber<uint64_t>(L, 2);
...
int LuaScriptInterface::luaPlayerSetMaxMana(lua_State* L)
{
	...
	Player* player = tfs::lua::getPlayer(L, 1);
	if (player) {
		player->manaMax = tfs::lua::getNumber<uint64_t>(L, 2);
		player->mana = std::min<uint64_t>(player->mana, player->manaMax);
		player->sendStats();
		tfs::lua::pushBoolean(L, true);
	} else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaNpcTypeHealth(lua_State* L)
{
	...
	if (npcType) {
		if (lua_gettop(L) == 1) {
			...
			uint64_t health = tfs::lua::getNumber<uint64_t>(L, 2);
...
int LuaScriptInterface::luaItemTypeGetAbilities(lua_State* L)
{
	...
	if (itemType) {
		...
		for (uint64_t i = 0; i < COMBAT_COUNT; i++) {
...
int LuaScriptInterface::luaCombatGetParameter(lua_State* L)
{
	...
	uint64_t value = combat->getParam(tfs::lua::getNumber<CombatParam_t>(L, 2));
...
int LuaScriptInterface::luaConditionGetParameter(lua_State* L)
{
	...
	uint64_t value = condition->getParam(tfs::lua::getNumber<ConditionParam_t>(L, 2));
	if (value == std::numeric_limits<int32_t>().max()) {
		lua_pushnil(L);
		return 1;
	}
...
int LuaScriptInterface::luaMonsterTypeHealth(lua_State* L)
{
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			monsterType->info.health = tfs::lua::getNumber<uint64_t>(L, 2);
			tfs::lua::pushBoolean(L, true);
		}
	} else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaMonsterTypeMaxHealth(lua_State* L)
{
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			monsterType->info.healthMax = tfs::lua::getNumber<uint64_t>(L, 2);
			tfs::lua::pushBoolean(L, true);
		}
	} else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaMonsterTypeRunHealth(lua_State* L)
{
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			monsterType->info.runAwayHealth = tfs::lua::getNumber<uint64_t>(L, 2);
			tfs::lua::pushBoolean(L, true);
		}
	} else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaMonsterSpellSetCombatValue(lua_State* L)
{
	...
	MonsterSpell* spell = tfs::lua::getUserdata<MonsterSpell>(L, 1);
	if (spell) {
		spell->minCombatValue = tfs::lua::getNumber<uint64_t>(L, 2);
		spell->maxCombatValue = tfs::lua::getNumber<uint64_t>(L, 3);
		tfs::lua::pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaMonsterSpellSetAttackValue(lua_State* L)
{
	...
	MonsterSpell* spell = tfs::lua::getUserdata<MonsterSpell>(L, 1);
	if (spell) {
		spell->attack = tfs::lua::getNumber<uint64_t>(L, 2);
...
int LuaScriptInterface::luaMonsterSpellSetConditionDamage(lua_State* L)
{
	...
	MonsterSpell* spell = tfs::lua::getUserdata<MonsterSpell>(L, 1);
	if (spell) {
		spell->conditionMinDamage = tfs::lua::getNumber<uint64_t>(L, 2);
		spell->conditionMaxDamage = tfs::lua::getNumber<uint64_t>(L, 3);
		spell->conditionStartDamage = tfs::lua::getNumber<uint64_t>(L, 4);
		tfs::lua::pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaWeaponHealth(lua_State* L)
{
	...
	Weapon* weapon = tfs::lua::getUserdata<Weapon>(L, 1);
	if (weapon) {
		weapon->setHealth(tfs::lua::getNumber<uint64_t>(L, 2));
		tfs::lua::pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaWeaponAttack(lua_State* L)
{
	...
	if (weapon) {
		uint16_t id = weapon->getID();
		ItemType& it = Item::items.getItemType(id);
		it.attack = tfs::lua::getNumber<uint64_t>(L, 2);
		tfs::lua::pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaWeaponMaxHitChance(lua_State* L)
{
	...
	if (weapon) {
		uint16_t id = weapon->getID();
		ItemType& it = Item::items.getItemType(id);
		it.maxHitChance = tfs::lua::getNumber<uint64_t>(L, 2);
		tfs::lua::pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
