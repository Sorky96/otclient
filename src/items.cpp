		...
		if (parseAttribute != ItemParseAttributesMap.end()) {
			...
			switch (parseType) {
				...
				case ITEM_PARSE_ATTACK: {
					it.attack = pugi::cast<uint64_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_HITCHANCE: {
					it.hitChance =
					    std::min<int8_t>(100, std::max<int8_t>(-100, pugi::cast<uint32_t>(valueAttribute.value())));
					break;
				}
				...
				case ITEM_PARSE_MANALEECHAMOUNT: {
					abilities.specialSkills[SPECIALSKILL_MANALEECHAMOUNT] = pugi::cast<uint64_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_MANALEECHCHANCE: {
					abilities.specialSkills[SPECIALSKILL_MANALEECHCHANCE] = pugi::cast<uint64_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_MAXHITPOINTS: {
					abilities.stats[STAT_MAXHITPOINTS] = pugi::cast<uint64_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_MAXHITPOINTSPERCENT: {
					abilities.statsPercent[STAT_MAXHITPOINTS] = pugi::cast<uint64_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_MAXMANAPOINTS: {
					abilities.stats[STAT_MAXMANAPOINTS] = pugi::cast<uint64_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_MAXMANAPOINTSPERCENT: {
					abilities.statsPercent[STAT_MAXMANAPOINTS] = pugi::cast<uint64_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_FIELDABSORBPERCENTENERGY: {
					abilities.fieldAbsorbPercent[combatTypeToIndex(COMBAT_ENERGYDAMAGE)] +=
					    pugi::cast<uint32_t>(valueAttribute.value());
					break;
				}
					...
					if (combatType != COMBAT_NONE) {
						...
						for (auto subAttributeNode : attributeNode.children()) {
							...
							if (tmpStrValue == "initdamage") {
								initDamage = pugi::cast<uint64_t>(subValueAttribute.value());
								...
								damage = -pugi::cast<uint64_t>(subValueAttribute.value());
								if (start > 0) {
									std::list<uint64_t> damageList;
									ConditionDamage::generateDamageList(damage, start, damageList);
									for (uint64_t damageValue : damageList) {
