...
// monsters
uint64_t Weapons::getMaxMeleeDamage(uint64_t attackSkill, uint64_t attackValue)
{
	return static_cast<uint64_t>(std::ceil((attackSkill * (attackValue * 0.05)) + (attackValue * 0.5)));
}

// players
uint64_t Weapons::getMaxWeaponDamage(uint32_t level, uint64_t attackSkill, uint64_t attackValue, float attackFactor)
{
...
{
	uint64_t damageModifier = playerWeaponCheck(player, target, item->getShootRange());
	if (damageModifier == 0) {
		return false;
	}
...
bool Weapon::useFist(Player* player, Creature* target)
{
	...
	uint64_t attackSkill = player->getSkillLevel(SKILL_FIST);
	uint64_t attackValue = 7;

	uint64_t maxDamage = Weapons::getMaxWeaponDamage(player->getLevel(), attackSkill, attackValue, attackFactor);
...
void Weapon::internalUseWeapon(Player* player, Item* item, Creature* target, uint64_t damageModifier) const
{
	if (scripted) {
		LuaVariant var;
...
void Weapon::onUsedWeapon(Player* player, Item* item, Tile* destTile) const
{
	...
	if (manaCost != 0) {
		...
		player->changeMana(-static_cast<uint64_t>(manaCost));
	}

	uint32_t healthCost = getHealthCost(player);
	if (healthCost != 0) {
		player->changeHealth(-static_cast<uint64_t>(healthCost));
	}
...
uint64_t Weapon::getHealthCost(const Player* player) const
{
	if (health != 0) {
		return health;
	}
...
uint64_t WeaponMelee::getElementDamage(const Player* player, const Creature*, const Item* item) const
{
	if (elementType == COMBAT_NONE) {
		return 0;
	}

	uint64_t attackSkill = player->getWeaponSkill(item);
	uint64_t attackValue = elementDamage;
	float attackFactor = player->getAttackFactor();

	uint64_t maxValue = Weapons::getMaxWeaponDamage(player->getLevel(), attackSkill, attackValue, attackFactor);
	return -normal_random(0, static_cast<uint64_t>(maxValue * player->getVocation()->meleeDamageMultiplier));
}

uint64_t WeaponMelee::getWeaponDamage(const Player* player, const Creature*, const Item* item,
                                     bool maxDamage /*= false*/) const
{
	uint64_t attackSkill = player->getWeaponSkill(item);
	uint64_t attackValue = std::max<uint64_t>(0, item->getAttack());
	float attackFactor = player->getAttackFactor();

	int32_t maxValue =
	    static_cast<uint64_t>(Weapons::getMaxWeaponDamage(player->getLevel(), attackSkill, attackValue, attackFactor) *
...
uint64_t WeaponDistance::getElementDamage(const Player* player, const Creature* target, const Item* item) const
{
	if (elementType == COMBAT_NONE) {
		return 0;
	}
	...
	uint64_t attackSkill = player->getSkillLevel(SKILL_DISTANCE);
	float attackFactor = player->getAttackFactor();
	...
	return -normal_random(minValue, static_cast<uint64_t>(maxValue * player->getVocation()->distDamageMultiplier));
}

uint64_t WeaponDistance::getWeaponDamage(const Player* player, const Creature* target, const Item* item,
                                        bool maxDamage /*= false*/) const
{
	uint64_t attackValue = item->getAttack();
...
uint64_t WeaponWand::getWeaponDamage(const Player*, const Creature*, const Item*, bool maxDamage /*= false*/) const
{
	if (maxDamage) {
		return -maxChange;
	}
