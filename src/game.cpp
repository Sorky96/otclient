...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		uint64_t realHealthChange = target->getHealth();
		...
		uint64_t healthChange = damage.primary.value + damage.secondary.value;
		if (healthChange == 0) {
			return true;
		}
		...
		if (targetPlayer &&
			...
			uint64_t manaDamage = std::min<uint64_t>(targetPlayer->getMana(), healthChange);
			if (damage.origin != ORIGIN_NONE) {
				...
				if (!events.empty()) {
					...
					manaDamage = std::min<uint64_t>(targetPlayer->getMana(), healthChange);
				}
			}
			...
			if (getBoolean(ConfigManager::MANASHIELD_BREAKABLE) && targetPlayer) {
				if (ConditionManaShield* conditionManaShield = dynamic_cast<ConditionManaShield*>(
					...
					if (uint64_t remainingManaDamage =
			...
			if (manaDamage != 0) {
				...
				if (damage.primary.value < 0) {
					damage.secondary.value = std::max<uint64_t>(0, damage.secondary.value + damage.primary.value);
					damage.primary.value = 0;
				}
			}
		}

		uint64_t realDamage = damage.primary.value + damage.secondary.value;
		if (realDamage == 0) {
			return true;
		}
		...
		uint64_t targetHealth = target->getHealth();
		if (damage.primary.value >= targetHealth) {
			damage.primary.value = targetHealth;
			damage.secondary.value = 0;
		} else if (damage.secondary.value) {
			damage.secondary.value = std::min<uint64_t>(damage.secondary.value, targetHealth - damage.primary.value);
		}
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	uint64_t manaChange = damage.primary.value + damage.secondary.value;
	if (manaChange > 0) {
		if (attacker) {
		...
		uint64_t realManaChange = targetPlayer->getMana();
		...
		uint64_t manaLoss = std::min<uint64_t>(targetPlayer->getMana(), -manaChange);
