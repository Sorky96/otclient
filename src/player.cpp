...
DepotLocker& Player::getDepotLocker()
{
	if (!depotLocker) {
		...
		for (uint32_t depotId = depotChest->capacity(); depotId >= 0; --depotId) {
                            ...
                            const Position& oldPos, bool teleport)
{
	...
	if (teleport || oldPos.z != newPos.z) {
		uint64_t ticks = getNumber(ConfigManager::STAIRHOP_DELAY);
		if (ticks > 0) {
...
void Player::onThink(uint32_t interval)
{
	...
	if (!getTile()->hasFlag(TILESTATE_NOLOGOUT) && !isAccessPlayer()) {
		idleTime += interval;
		const uint64_t kickAfterMinutes = getNumber(ConfigManager::KICK_AFTER_MINUTES);
...
void Player::removeMessageBuffer()
{
	...
	const uint64_t maxMessageBuffer = getNumber(ConfigManager::MAX_MESSAGEBUFFER);
...
void Player::drainHealth(Creature* attacker, uint64_t damage)
{
	Creature::drainHealth(attacker, damage);
	sendStats();
}

void Player::drainMana(Creature* attacker, uint64_t manaLoss)
{
	onAttacked();
	changeMana(-manaLoss);
...
void Player::removeExperience(uint64_t exp, bool sendText /* = false*/)
{
	...
	while (level > 1 && experience < currLevelExp) {
		--level;
		healthMax = std::max<uint64_t>(0, healthMax - vocation->getHPGain());
		manaMax = std::max<uint64_t>(0, manaMax - vocation->getManaGain());
...
BlockType_t Player::blockHit(Creature* attacker, CombatType_t combatType, uint64_t& damage,
                             ...
                             bool ignoreResistances /* = false*/)
{
	...
	if (!ignoreResistances) {
		...
		for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; ++slot) {
			...
			const uint32_t& absorbPercent = it.abilities->absorbPercent[combatIndex];
			if (absorbPercent != 0) {
			...
			if (field) {
				const uint32_t& fieldAbsorbPercent = it.abilities->fieldAbsorbPercent[combatIndex];
...
uint64_t Player::getGainedExperience(Creature* attacker) const
{
	if (getBoolean(ConfigManager::EXPERIENCE_FROM_PLAYERS)) {
		...
		if (attackerPlayer && attackerPlayer != this && skillLoss &&
		    std::abs(static_cast<uint64_t>(attackerPlayer->getLevel() - level)) <=
...
void Player::onAttackedCreatureDrainHealth(Creature* target, uint64_t points)
{
...
void Player::onTargetCreatureGainHealth(Creature* target, uint64_t points)
{
	if (target && party) {
...
void Player::changeHealth(uint64_t healthChange, bool sendHealthChange /* = true*/)
{
	Creature::changeHealth(healthChange, sendHealthChange);
	sendStats();
}

void Player::changeMana(uint64_t manaChange)
{
	if (!hasFlag(PlayerFlag_HasInfiniteMana)) {
		if (manaChange > 0) {
			mana += std::min<uint64_t>(manaChange, getMaxMana() - mana);
		} else {
			mana = std::max<uint64_t>(0, mana + manaChange);
		}
	}
...
{
	uint64_t deathLosePercent = getNumber(ConfigManager::DEATH_LOSE_PERCENT);
