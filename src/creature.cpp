...
void Creature::changeHealth(uint64_t healthChange, bool sendHealthChange /* = true*/)
{
	uint64_t oldHealth = health;

	if (healthChange > 0) {
		health += std::min<uint64_t>(healthChange, getMaxHealth() - health);
	} else {
		health = std::max<uint64_t>(0, health + healthChange);
	}
...
void Creature::gainHealth(Creature* healer, uint64_t healthGain)
{
...
void Creature::drainHealth(Creature* attacker, uint64_t damage)
{
...
BlockType_t Creature::blockHit(Creature* attacker, CombatType_t combatType, uint64_t& damage,
...
void Creature::addDamagePoints(Creature* attacker, uint64_t damagePoints)
{
	if (damagePoints <= 0) {
		return;
	}
...
void Creature::onAttackedCreatureDrainHealth(Creature* target, uint64_t points)
{
...
bool FrozenPathingConditionCall::operator()(const Position& startPos, const Position& testPos,
                                            const FindPathParams& fpp, uint64_t& bestMatchDist) const
{
