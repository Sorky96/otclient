...
CombatDamage Combat::getCombatDamage(Creature* creature, Creature* target) const
{
	...
	if (formulaType == COMBAT_FORMULA_DAMAGE) {
		damage.primary.value = normal_random(static_cast<uint64_t>(mina), static_cast<uint64_t>(maxa));
	} else if (creature) {
		int32_t min, max;
...
uint64_t Combat::getParam(CombatParam_t param)
{
	switch (param) {
		case COMBAT_PARAM_TYPE:
			return static_cast<uint64_t>(params.combatType);
...
void Combat::doTargetCombat(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
	...
	if (success) {
		...
		if (!damage.leeched && damage.primary.type != COMBAT_HEALING && casterPlayer &&
			...
			uint64_t totalDamage = std::abs(damage.primary.value + damage.secondary.value);
                          ...
                          const CombatParams& params)
{
	...
	for (Creature* creature : toDamageCreatures) {
		...
		if (success) {
			...
			uint64_t totalDamage = std::abs(damageCopy.primary.value + damageCopy.secondary.value);

			if (casterPlayer && !damage.leeched && damage.primary.type != COMBAT_HEALING &&
			    damage.origin != ORIGIN_CONDITION) {
				uint64_t targetsCount = toDamageCreatures.size();
...
void ValueCallback::getMinMaxValues(Player* player, CombatDamage& damage) const
{
	...
	if (lua_pcall(L, parameters, 2, 0) != 0) {
		...
		damage.primary.value = normal_random(tfs::lua::getNumber<uint64_t>(L, -2), tfs::lua::getNumber<uint64_t>(L, -1));
		lua_pop(L, 2);
	}
...
void AreaCombat::setupArea(uint64_t length, uint64_t spread)
{
...
void AreaCombat::setupArea(uint64_t radius)
{
...
void AreaCombat::setupAreaRing(uint64_t ring)
{
