...
void Monster::onCreatureLeave(Creature* creature)
{
	...
	if (isOpponent(creature)) {
		...
		if (!isSummon() && targetList.empty()) {
			uint64_t walkToSpawnRadius = getNumber(ConfigManager::DEFAULT_WALKTOSPAWNRADIUS);
...
BlockType_t Monster::blockHit(Creature* attacker, CombatType_t combatType, uint64_t& damage,
                              ...
                              bool /* ignoreResistances = false */)
{
	...
	if (damage != 0) {
		...
		if (elementMod != 0) {
			damage = static_cast<uint64_t>(std::round(damage * ((100 - elementMod) / 100.)));
			if (damage <= 0) {
				damage = 0;
...
bool Monster::getCombatValues(uint64_t& min, uint64_t& max)
{
...
void Monster::updateLookDirection()
{
	...
	if (attackedCreature) {
		...
		uint64_t offsetx = pos.getOffsetX(attackedCreaturePos);
		uint64_t offsety = pos.getOffsetY(attackedCreaturePos);
...
void Monster::drainHealth(Creature* attacker, uint64_t damage)
{
...
void Monster::changeHealth(uint64_t healthChange, bool sendHealthChange /* = true*/)
{
