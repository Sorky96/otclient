...
public:
	...
	virtual uint64_t getMaxHealth() const { return healthMax; }
	...
	virtual BlockType_t blockHit(Creature* attacker, CombatType_t combatType, uint64_t& damage,
	...
	virtual void changeHealth(uint64_t healthChange, bool sendHealthChange = true);

	void gainHealth(Creature* healer, uint64_t healthGain);
	virtual void drainHealth(Creature* attacker, uint64_t damage);
	...
	void addDamagePoints(Creature* attacker, uint64_t damagePoints);
	...
	virtual void onAttackedCreatureDrainHealth(Creature* target, uint64_t points);
	virtual void onTargetCreatureGainHealth(Creature*, uint64_t) {}
	...
	virtual bool getCombatValues(uint64_t&, uint64_t&) { return false; }
...
protected:
	...
	int32_t varSpeed = 0;
	uint64_t health = 1000;
	uint64_t healthMax = 1000;
